...
var sysBBS = "<span style='font-size:14px; line-height:30px;'>欢迎光临心之语聊天室，请遵守聊天室规则，不要使用不文明用语。</span><br><span style='line-height:22px;'>";var sysBBS = "<span style='font-size:14px; line-height:30px;'>欢迎光临心之语聊天室，请遵守聊天室规则，不要使用不文明用语。</span><br><span style='line-height:22px;'>";
	window.setInterval("showContent();",1000);//定时执行
	window.setInterval("showOnLine();",10000);//定时执行
	window.setInterval("check();",1000);//定时执行
	// Jquery:JS框架.
	// 相当于window.onload
	//这个方法，页面一加载，方法就会执行。
	$(function(){
	...
	// 显示聊天的内容
	//new Date().getTime(),{'method':'getMessage'},function(data)
	//这里的new Date()是防止缓存，然后就进入user(userServlet)中执行getMessage方法，返回后，就会执行function(data)方法。
	...
	<table width="778" height="95" border="0" align="center"
		...
		<form action="" id="form1" name="form1" method="post">
			...
			<tr>
				...
				<!-- if(event.keyCode==13 && event.ctrlKey)就是回车键 -->
			...
			<tr>
				...
				<td colspan="2" align="center" class="word_dark">&nbsp;All
					CopyRights Reserved 2018 广东财经大学信息学院汤海麟</td>
				<td align="center">&nbsp;</td>
			</tr>
		</form>
	</table>
