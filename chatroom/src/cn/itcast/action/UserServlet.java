...
public class UserServlet extends BaseServlet {
	...
	public String sendMessage(HttpServletRequest req,HttpServletResponse resp) throws IOException{
		...
		String content = req.getParameter("content"); // 发言内容（单纯内容），下面的sourceMessage是指包括发送人，接收人，表情，内容等信息的综合体
	...
	public String login(HttpServletRequest req,HttpServletResponse resp){
		...
		try {
			...
			if (existUser == null) {
				...
				//用户登录成功
				//第一个bug：在同一个浏览器登录不同的账号，会用同一个session。
				//解决方法：第二个账号登录的时候，把第一个的session销毁掉
				
				// 第一个BUG的解决:第二个用户登录后将之前的session销毁!
				req.getSession().invalidate();
				
				/*第二个Bug:在不同的浏览器登录同一个账号，会有两个不同的session，
				账号第一次登录时的session，会被账号第二次登录的session替换掉，
				因为在map中，他们具有相同的key，所以第二次登录的session会把第一次的session
				替换掉。*/
				
				/*解决方法：登录的时候先判断该用户是否已存在map中，如果存在，就先把第一次登录的session
				销毁掉，然后再加入第二次登录的session*/
				Map<User, HttpSession> userMap = (Map<User, HttpSession>) getServletContext()
						.getAttribute("userMap");
				// 判断用户是否已经在map集合中'
